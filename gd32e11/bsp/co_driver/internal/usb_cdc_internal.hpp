...
struct cdc_usr; // 底层句柄（定义于 co_usb_cdc.cpp ）

// 发送 / 接收节点（继承 worknode 的 intrusive 结构）
struct cdc_usr_node : co_wq::worknode {
    uint8_t* data;    // 数据缓冲
    uint16_t dat_max; // 期望长度
    uint16_t dat_cur; // 已完成长度
};
...
// 挂接异步 I/O 节点（内部可能立即触发硬件传输，需在持锁情况下调用）
