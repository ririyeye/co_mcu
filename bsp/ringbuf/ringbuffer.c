...
{
    return rb_size(fifo) - rb_len(fifo);
}
 ...
 *         of bytes copied.
 */
uint32_t rb_in(ringbuffer_t* fifo, const void* datptr, uint32_t len)
{
 ...
 */
uint32_t rb_out(ringbuffer_t* fifo, void* outbuf, uint32_t len)
{
